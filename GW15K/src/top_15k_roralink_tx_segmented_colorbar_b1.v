// ============================================================================
// 15K RoraLink 8B10B TX-only Segmented Video Colorbar Source - B1
// ----------------------------------------------------------------------------
// Purpose:
//   Generate segmented 1080p RGB888_32 colorbar packets and transmit them over
...
//           flags[1] = EOF segment: line1079/seg7
//           flags[2] = EOL segment: seg7
//           flags[3] = SOL segment: seg0
//   word4..last : pixel payload, one 32-bit word per pixel
//
// Segmentation:
//   1920 pixels/line -> 8 segments/line
//   seg0..seg6: 256 pixels each
//   seg7      : 128 pixels
// ============================================================================
...
// --------------------------------------------------------------------------
(* keep = "true" *) wire [31:0] ila15v_tx_top_version = 32'h1506_3002;
...
// Approximate 1080p60 line pacing at 6.25G user clock.
// Per line words = 8*4 headers + 1920 payload = 1952.
// 156.25MHz / (1080*60) ~= 2411 cycles/line.
localparam [11:0] LINE_GAP_CYCLES    = 12'd432;
...
wire [11:0] x_base        = {segment_id[3:0], 8'h00}; // segment_id * 256, valid for 0..7
...
function [23:0] colorbar_rgb;
    ...
    input [10:0] y;
    begin
        // 8 vertical bars, 240 pixels each. y is reserved for future patterns.
        if (x < 12'd240)
